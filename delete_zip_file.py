...
class Zipfile2JsonL:
    ...
    def static_file_info(self):
        ...
        for key in info:
            ext_info = info[key]
            avg_size = float(ext_info["size"]) / (float(ext_info["num"] + 0.000001))
            ext_info["avg_size"] = round(avg_size, 2)
            if ext_info["avg_size"] > self.max_file_size:
                self.useless_ext_list.append(key)
        return info

    def detect_char_encode(self, content: bytes):
            ...
            for info in zf.infolist():
                if not info.is_dir():
                    ...
                    with zf.open(info, 'r') as file:
                        ...
                        encoding = self.detect_char_encode(content)
                        text = None
                        # 在遍历zipfile的时候做一层过滤收集无用文件
    ...
    def dump_to_jsonl(self, output_file_path, ext_info=None, with_ext_info=False, with_text=False):
    ...
    def remove_useless_file(self):
        # update useless file by useless_ext_list （collect by static ext info）
    ...
    def __call__(self, meta_info_path, file_json_path, delete_non_text=False):
        try:
            ...
            if meta_info_path is not None:
                self.dump_to_jsonl(meta_info_path, ext_info=ext_file_info, with_ext_info=True, with_text=False)
            if delete_non_text:
                self.remove_useless_file()
            if file_json_path is not None:
                self.dump_to_jsonl(file_json_path, with_text=True)
            logger.info(ext_file_info)
            return ext_file_info
        except Exception as e:
            print_exc()
            logger.error(f"处理过程中发生错误: {str(e)}")
            return None
...
def process_zips(root_dir, save_root):
    ...
    for file in file_list:
        ...
        if not json_file_path.exists():
            ...
            meta_file_path = json_file_path.with_suffix(".meta.jsonl")
            handler(meta_info_path=meta_file_path, file_json_path=json_file_path, delete_non_text=True)
